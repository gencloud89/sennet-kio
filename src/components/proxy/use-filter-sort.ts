...
// SENNET VPN: filter out info-nodes embedded in subscription YAML
// Server embeds account info as fake proxies with emoji names or "Reset" prefix
const INFO_NODE_PREFIXES = ["👤", "📝", "📨", "⏳"];
export function isInfoNode(name: string): boolean {
  if (!name) return false;
  if (name.startsWith("Reset") || name.startsWith("reset")) return true;
  return INFO_NODE_PREFIXES.some((p) => name.startsWith(p));
}

export function filterInfoNodes(proxies: IProxyItem[]): IProxyItem[] {
  return proxies.filter((p) => !isInfoNode(p.name));
}
...
function filterProxies(
  ...
  // Always remove SENNET info-nodes before any other filtering
  const filtered = filterInfoNodes(proxies);
  const query = filterText.trim();
  if (!query) return filtered;
  // re-bind proxies to filtered list for subsequent logic
  proxies = filtered;
